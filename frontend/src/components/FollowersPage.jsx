...
const FollowersPage = () => {
  ...
  const unfollowHandler = async (username,userId) => {
    try {
      ...
      if (res.data.success) {
        toast.success(`${username} removed from your followers`);
        
        // let updatedFollowers;
        // if (followers?.includes(userId)) {
        //   toast.success("User Unfollowed");
        //   updatedFollowers = followers?.filter((item) => item !== userId);
        // } else {
      ...
      <Table>
        ...
        {followers && followers?.map((item,index)=>(
            ...
            <TableRow>
              ...
              <Avatar>
                    <AvatarImage src={item?.profilePicture} alt="avatar" className="w-[40px] h-[40px] rounded-full"/>
                    <AvatarFallback>CN</AvatarFallback>
                </Avatar>
              ...
              <TableCell className="text-right">
                {user._id === item._id ? (
                  ...
                  <button className="border-[2px] border-blue-400 py-1 px-3 font-semibold rounded-[5px] text-blue-400 hover:bg-blue-400 hover:text-white" onClick={()=>unfollowHandler(item?.username,item._id)}>{user?.following.includes(item._id) ? "Unfollow" : "Follow"}</button>
                )}
